...
test('it test userPage render', async()=>{
    ...
    await controller.userPage({sessions:{error:[]}},{render:mockRender});
...
test('it test registerUserForm render', async()=>{
    const mockRender = jest.fn();
    await controller.registerUserForm({sessions:{error:[]}},{render:mockRender});
    expect(mockRender).toHaveBeenCalledTimes(1);
...
test('it test registerUser controller', async()=>{
    ...
    const userData = {
      ...
      await controller.registerUser({sessions:{error:[]}, body: userData},{redirect:mockRedirect});
...
test('it test editUser controller', async()=>{
    ...
    await controller.editUser({params: {id:1}, sessions: {error:[]}, body: userData2}, { redirect: mockRedirect });
...
test('it test deleteUser controller', async()=>{
    const mockRedirect = jest.fn();
    await controller.deleteUser({params: {id:1}}, { redirect: mockRedirect });
